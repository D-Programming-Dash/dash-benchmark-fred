...
{
    /+ Dash: We hardcode the file name.
    if(args.length < 2)
    {
    ...
    +/
    enum inputFileName = "dna-test-file";

    auto stripper = regex(`>.*?\n|\n`, "g");
    auto data = cast(string)std.file.read(inputFileName);
    auto stripped = std.regex.replace!matchFn(data, stripper, "");
    foreach(p; patterns)
    {
        version(ct_regex)
    ...
    foreach(i, p; patterns2)
    {
        static if (!(i & 1))
        {
            version(ct_regex)
                ...
                auto reg = regex(p, "g");
            replaced = std.regex.replace!matchFn(replaced, reg, patterns2[i+1]);
        }
    }
